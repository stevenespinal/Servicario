...
export const createCollaboration = col =>
  db.collection("collaborations").add(col).then(docRef => docRef.id);

export const sendMessage = msg => db.collection("profiles").doc(msg.toUser).collection("messages").add(msg);
export const subscribeToMessages = (userId, callback) =>
  db.collection("profiles").doc(userId).collection("messages").onSnapshot(snapshot => {
    const messages = snapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
    callback(messages);
  });

export const markMessageAsRead = msg =>
  db.collection("profiles").doc(msg.toUser).collection("messages").doc(msg.id).update({isRead: true});

export const fetchCollaborations = userId =>
  db.collection("collaborations").where("allowedPeople", 'array-contains', userId).get()
    .then(snapshot => snapshot.docs.map(doc => ({id: doc.id, ...doc.data()})));

export const subscribeToCollaboration = (collaborationId, callback) =>
  db.collection("collaborations").doc(collaborationId).onSnapshot(snapshot => {
...
export const sendCollabMessage = ({message, collabId, timestamp}) =>
  db.collection("collaborations").doc(collabId).collection("messages").doc(timestamp).set(message);
